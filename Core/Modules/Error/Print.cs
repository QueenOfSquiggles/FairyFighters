...
public static class Print {
  ...
  private static string StripMetaFromType<T>(T inst) => (inst?.GetType()?.FullName ?? "null") +
    (inst is Node node ?
#if DEBUG
    ...
    $"'{node?.GetPath() ?? "nullPath"}' -> OwnerScene({node?.Owner?.SceneFilePath ?? "null"})"
#else
    $"'{node?.Name ?? "null"}'" // just do the name in non-debug environments
#endif
    : "");
